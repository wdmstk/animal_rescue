...
## 6.3 CI/CD 品質ゲート自動スクリプト仕様 (Tooling Spec)
GitHub Actions や Pull Request チェック時に以下の検証スクリプトを自動実行し、品質ゲートの遵守を保証する。

```bash
# 1. ブランド禁止語および文言チェック (Lint)
npx textlint --rule prohibit-words docs/ src/

# 2. ドキュメント相互参照リンク切れチェック
npx markdown-link-check docs/foundation/*.md

# 3. AI 監査ログ JSON Schema 自動バリデーション
npx ajv-cli validate -s docs/schema/ai_audit_log.schema.json -d src/logs/sample_audit.json

# 4. バージョン整合性チェック (親文書と子文書の版ずれ検知)
node scripts/check-doc-version-consistency.js
```
...
| 1.4 | 2026-07-24 | 100点品質化改訂：親文書参照バージョンの適正化(DESIGN_SYSTEM.md v2.1.3)およびCI/CD品質ゲート自動スクリプト仕様(6.3)の追加 |
