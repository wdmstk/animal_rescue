...
3. `TASK-302` 問い合わせ環境（チケット制サポート）のユーザー側UI（/support）の実装
...
- Task ID: `TASK-325`
- GitHub Issue: #278
- ブランチ: `fix/TASK-325-settings-and-profile-editor-fixes`
- ステータス: `done`
- 依存関係: なし
- 概要:
  - (1) 飼い主情報（OwnerProfile）がDB未登録の初期状態でも設定画面で編集・保存できるように改善
  - (2) ペット基本情報編集画面（PetProfileEditorCard/FormField）のglobals.cssワイルドカード属属性セレクタ干渉による文字読みにくさを修復し、視認性の高いダークテーマデザインに統一
  - (3) 課金ポータルAPI（/api/billing/portal）での例外発生・stripeCustomerId未登録時の顧客作成＆フォールバック処理を修復し、画面遷移を正常化
- 受入条件:
  - 飼い主情報が未登録のユーザーでも設定画面から正常に飼い主情報を登録・保存できる
  - 基本情報編集画面のテキスト・ラベル・入力欄がクッキリと読める高コントラストスタイリングになっている
  - 「契約を管理する」ボタンを押した際にStripeポータルへのリダイレクトが正常に機能する（未登録時も顧客作成により対応）
  - Vitestテスト、npm run lint、E2Eテストがパスする

### 法的情報ページUI/UX配色視認性改善＆アプリ名「AniLink」一括変更
- Task ID: `TASK-322`
- GitHub Issue: #272
...
- 依存関係: なし
- 完了条件:
  - 法的4ページ (/legal/terms, /legal/privacy, /legal/commercial, /legal/cookie) の視認性・配色デザイン改善
  - アプリ名称「AniLink」の一括適用
  - `npm run lint`, `npx vitest run`, `npm run check:brand` 全緑確認
...
- Task ID: `TASK-318`
- GitHub Issue: #266
- ブランチ: `feat/TASK-318-settings-admin-and-user-announcements`
- ステータス: `done`
- 概要: （1）設定画面の「家族招待コード」以外のカード（オーナープロフ、表示設定、世帯メンバー、危険領域など）のテキスト文字色・背景コントラストを完全修復。（2）Admin管理画面群（`/admin` 全ページ）の配色を白背景から通常のダークテーマ（ガラスモフィズム・グラデーション・ネオンバッジ）へ統一。（3）Adminが発報する「お知らせ・通知」をユーザーが確認するための公開API `/api/announcements` およびユーザーダッシュボード上の通知カード/ポップアップ表示コンポーネントを実装。
- 依存関係: なし
- 完了条件:
  - 設定画面の全カードタイトル・見出し・説明文のコントラストがWCAG AA準拠でハッキリ見える
  - `/admin` 配下の全画面が通常画面と同じダークモダンスタイルで美しく統一される
  - 管理者が登録したお知らせが公開され、ユーザー側ダッシュボード/ヘッダーで確認・閲覧可能
  - `npm run lint`, `npx vitest run`, `npm run test:e2e` が全てグリーン



### UI/UX第2弾改善（医療のみサブタブ化・サマリー印刷レイアウト修正・投薬/ワクチン/管理コントラスト修正・履歴表示改善）
- Task ID: `TASK-317`
- GitHub Issue: #264
- ブランチ: `feat/TASK-317-ui-ux-refinement-round2`
- ステータス: `done`
- 概要: ユーザーフィードバックに基づくUI/UX追加改善。（1）基本情報・緊急・管理タブのサブタブ解除と医療タブのみサブタブ保持、（2）通院サマリーの縦1列可読レイアウト化、（3）印刷時のエクスポートカード非表示＆トップカード重なり排除、（4）投薬・ワクチン・管理タブの文字コントラスト完全修復、（5）健康記録最下部履歴の1列整列・改行防止レイアウト化。
- 依存関係: なし
- 完了条件:
  - 基本情報・緊急・管理タブが全項目縦並び、医療タブのみサブタブ切り替え可能
  - 通院サマリーの基本プロフと緊急情報が縦並びで可読性向上
  - 印刷時（`@media print`）に提出サマリー以外のカードや印刷ボタン、余計なヘッダー重なりが一切発生せず100%クリーン印刷
  - 投薬・ワクチン・管理タブの見出し・ラベル・サブ文字のコントラストがWCAG AAを満たす
  - 健康記録の各履歴が適切な幅と1行レイアウトで改行なく整然と表示される
  - `npm run lint`, `npx vitest run`, `npm run test:e2e` が全てグリーン



### UI/UX包括的改善（文字コントラスト視認性・拡張項目UI・提出サマリー印刷・医療タブ構造分離）
- Task ID: `TASK-316`
- GitHub Issue: #260
- ブランチ: `feat/TASK-316-ui-ux-comprehensive-improvements`
- ステータス: `done`
- 概要: タイトル・テキストの文字コントラスト視認性不具合、推移グラフ非選択ボタン等の見づらさ、拡張項目フォーム見切れとガイド不足、提出サマリーの常時表示問題と印刷時空ページバグ、および医療タブの情報過多スクロール問題を包括的に修正・改善する。
- 依存関係: なし
- 完了条件:
  - 基本情報・医療タブ等におけるカードタイトル、推移グラフ見出し、非選択ボタン、履歴見出し等の文字コントラスト・視認性を完全修正 (WCAG 2.1 AA準拠)
  - 拡張項目フォームのレスポンシブ幅・プレースホルダー見切れ解消、使い方のガイドテキスト追加
  - 提出サマリーの全タブ常時表示を整理し独立タブ/ボタン化、`@media print` 印刷時の空ページ発生バグを完全解消
  - 医療タブの「一括縦並び表示」を「個別サブタブ切り替え (セグメンテッドコントロール)」化し、情報の到達性向上とスクロール長大化を解消
  - UI/UX検証用のPlaywright E2Eテストを追加し、`npm run lint`, `npx vitest run`, `npm run test:e2e` が全てグリーンであること



### デザイントークン正規化とglobals.cssオーバーライドの撤去
- Task ID: `TASK-310`
- GitHub Issue: #249
...
- 依存関係: なし
- 完了条件:
  - `DESIGN_TOKENS.md` 準拠のCSS変数（`--color-brand-primary`, `--color-surface-card` 等）を定義
...
- 完了条件:
  - `vitest-axe` / `axe-playwright` 等によるアクセシビリティ自動チェックの導入
  - ブランドコピー禁止語（「絶対」「100%」等）・煽り表現の静的検証スクリプトの追加
  - `.github/workflows/ci.yml` への Quality Gate ステップ追加
  - `npm run lint` / `npx vitest run` が通ること


### オフラインデータ競合解決ダイアログの実装
- Task ID: `TASK-313`
- GitHub Issue: #252
...
- 依存関係: `TASK-311`
- 完了条件:
  - サーバー vs ローカルデータの差分表示とマージ選択 UI (`ConflictResolverDialog`) の実装
  - データ上書き時の `AuditLog` への旧データ退避ロジック
  - `npm run lint` / `npx vitest run` が通ること


### Brand × UX × AI 統合UI標準化（AI確信度表示・緊急モード修復）
- Task ID: `TASK-312`
- GitHub Issue: #251
...
- 完了条件:
  - AI提案・抽出結果カード (`AiProposalCard`) への確信度 (Confidence Level)・免責事項・根拠メタデータの提示
  - 緊急公開画面 (`emergency-public-view.tsx`) の `animate-pulse` や過度な赤グラデーションの除去と60文字以内3ステップ構造への修復
  - `npm run lint` / `npx vitest run` が通ること


### 共通UIコンポーネント基盤の拡充とアクセシビリティ標準化
- Task ID: `TASK-311`
- GitHub Issue: #250
...
- 依存関係: `TASK-310`
- 完了条件:
  - 生 `<input>`, `<textarea>`, Card, Badge, Alert 等の共通 UI コンポーネント化
  - `aria-live`, `aria-invalid`, `aria-describedby` およびフォーカスリングの標準組み込み
  - `npm run lint` / `npx vitest run` が通ること
...
- 依存関係: なし
- 完了条件:
  - LPの `/lp` ルートを作成し、各種セクションを実装。
  - 実データに対応したモックアップ画像を public/ に上書き配置。
  - `STRIPE_PRICE_ID_ANNUAL_7800` を `STRIPE_PRICE_ID_ANNUAL_6800` へとコード/テスト全体でリネーム。
  - supabase serverクライアントでのクッキー書き込みtry-catch処理を追加。
  - すべてのテストとLintがグリーンであること。

### ダッシュボードおよび管理画面のUI洗練化・テーマ一本化
- Task ID: `TASK-305`
- ブランチ: `refactor/TASK-305-dashboard-refinement`
- ステータス: `done`
- 概要: 不要なテーマ切り替え機能およびテストを削除し、ダッシュボード全体をLPのトーンであるグラスモーフィズム調のダークテーマに一本化・洗練する。
- 依存関係: なし
- 完了条件:
  - テーマ切り替えボタン（ThemeToggle）の削除と、globals.cssのテーマ固定化。
  - ペット詳細画面（/pets/[petId]）のレイアウトおよび各管理カードのUI洗練化。
  - 不要となったテーマ切り替え関連のテスト（e2e/unit）の削除。
  - `npm run lint` および `npx vitest run` / `npm run test:e2e` が正常に通過する。

### LPの生成AIイメージに合わせたUI洗練化（ダッシュボード・緊急公開画面）
- Task ID: `TASK-304`
- ブランチ: `refactor/TASK-304-ui-refinement`
- ステータス: `done`
- 概要: LPの生成AIイメージのトーン（ダークテーマ、グラスモーフィズム、高級感）に合わせて実際のアプリ画面のUIを改善する。
- 依存関係: なし
- 完了条件:
  - ダッシュボード（/pets）のスタイルをモダンな構成に調整。
  - ペットカードをグラスモーフィズムやホバー演出を取り入れた高級感のあるデザインへ変更。
  - 緊急公開画面（/e/[token]）をダーク調で視認性の高い、プレミアムなデザインへ変更。
  - `npm run lint` および `npx vitest run` が正常に通過する。
