...
### in_progress
1. `TASK-162` Empty State改善（次アクション導線つき）

### todo
1. `TASK-165` データ共有出力（PDF/印刷向け）
2. `TASK-164` 変更履歴（監査ログ）表示
3. `TASK-163` 通知設定UI（投薬リマインダー実利用化）
4. `TASK-159` ペット詳細ページの情報密度最適化（セクションナビ）
5. `TASK-158` 緊急公開画面の視認性改善（救急モードUI）
...
4. `TASK-161` ローディング/スケルトンの統一
5. `TASK-160` 共通フィードバックUI（成功/失敗トースト）導入
