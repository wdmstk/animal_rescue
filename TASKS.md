...
2. `TASK-156` 設定表示APIのPrisma delegate未生成時500エラー修正
3. `TASK-155` 検査/設定修正 + 拡張項目自由化（複数指定）
4. `TASK-154` ペット詳細表示時のコンソールエラー修正
5. `TASK-153` PRマージ前チェックをCI実行結果と連動強制
6. `TASK-152` 検査カテゴリ基盤再設計（DB/API/型）
7. `TASK-151` 健康記録UIをカテゴリ分離 + 尿項目段階導入
8. `TASK-150` ペット単位表示設定のデータモデル/API実装
9. `TASK-149` 設定画面での表示ON/OFF管理UI
10. `TASK-148` 緊急公開画面への追加表示（直近サマリー）反映
11. `TASK-147` 設定画面のログイン情報更新失敗を修正
12. `TASK-146` 課金機能（Stripe定期課金MVP・月額500円）
13. `TASK-145` 家族情報＋ログイン情報の表示/追加/編集（権限管理込み）
14. `TASK-144` ログアウト機能（UI導線）
15. `TASK-143` ペット詳細ページ Runtime Error 修正
16. `TASK-142` テストデータ基盤整備（UI確認＋自動テスト両立）
17. `TASK-141` PRチェックCI再発火最適化とセルフレビュー3観点必須化
18. `TASK-140` PRチェックのマージ前/マージ後CI検証追加
19. `TASK-139` PR運用チェック項目のタイムライン整備
20. `TASK-138` 保護APIの認証・所有境界チェック強化
21. `TASK-137` ペット一覧/詳細の実データ化（モック撤去）
22. `TASK-136` ペット新規登録導線
23. `TASK-135` ペット基本情報編集導線
24. `TASK-134` 緊急情報・投薬の編集導線実装
25. `TASK-133` 家族招待コード発行・共有導線
26. `TASK-132` ログイン失敗時のランタイム例外表示修正
27. `TASK-131` 新規ユーザー登録導線の追加
28. `TASK-130` 公開緊急RPC未配置時のフォールバック対応
