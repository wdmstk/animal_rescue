...
### done
1. `TASK-101` ドキュメント整備
2. `TASK-102` 統合テスト（unit/integration/e2e）
3. `TASK-103` ワクチン・予防歴
4. `TASK-104` 投薬管理 + カレンダー
5. `TASK-105` 医療タイムライン
6. `TASK-106` 緊急情報 + QR公開画面
7. `TASK-107` ペットプロフィール + 写真管理
8. `TASK-108` スキーマ + RLS実装
9. `TASK-109` 認証 + 家族招待コード基盤
...
17. `TASK-117` 投薬リマインダー実送信
18. `TASK-118` ワクチン履歴編集フロー
19. `TASK-119` 実DB統合テスト + CI記載更新
20. `TASK-120` 健康トラッキングAPI/スキーマ整備
21. `TASK-121` 健康トラッキングの入力UI
22. `TASK-122` 健康トラッキングのグラフ可視化UI
23. `TASK-123` 健康トラッキングのテスト拡充（unit/integration/e2e）
24. `TASK-124` Prisma adapter未設定による500エラー修正
25. `TASK-125` health APIのpetId UUIDバリデーション強化（500→400）
26. `TASK-126` 実装運用の必須ガード（AGENTS/PRテンプレ/CI）
27. `TASK-127` 公開緊急導線の整合化（QRトークン/ドキュメント/E2E）
28. `TASK-128` TASK作成時のIssue必須化と並行実装向け分割方針の追加
29. `TASK-129` TASK完了時のIssueクローズ必須化
30. `TASK-130` 公開緊急RPC未配置時のフォールバック対応
31. `TASK-131` 新規ユーザー登録導線の追加
32. `TASK-132` ログイン失敗時のランタイム例外表示修正
33. `TASK-133` 家族招待コード発行・共有導線
34. `TASK-134` 緊急情報・投薬の編集導線実装
35. `TASK-135` ペット基本情報編集導線
36. `TASK-136` ペット新規登録導線
37. `TASK-137` ペット一覧/詳細の実データ化（モック撤去）
38. `TASK-138` 保護APIの認証・所有境界チェック強化
39. `TASK-139` PR運用チェック項目のタイムライン整備
40. `TASK-140` PRチェックのマージ前/マージ後CI検証追加
41. `TASK-141` PRチェックCI再発火最適化とセルフレビュー3観点必須化
42. `TASK-142` テストデータ基盤整備（UI確認＋自動テスト両立）
43. `TASK-143` ペット詳細ページ Runtime Error 修正
44. `TASK-144` ログアウト機能（UI導線）
45. `TASK-145` 家族情報＋ログイン情報の表示/追加/編集（権限管理込み）
46. `TASK-146` 課金機能（Stripe定期課金MVP・月額500円）
47. `TASK-147` 設定画面のログイン情報更新失敗を修正
48. `TASK-148` 緊急公開画面への追加表示（直近サマリー）反映
49. `TASK-149` 設定画面での表示ON/OFF管理UI
50. `TASK-150` ペット単位表示設定のデータモデル/API実装
51. `TASK-151` 健康記録UIをカテゴリ分離 + 尿項目段階導入
52. `TASK-152` 検査カテゴリ基盤再設計（DB/API/型）
53. `TASK-153` PRマージ前チェックをCI実行結果と連動強制
54. `TASK-154` ペット詳細表示時のコンソールエラー修正
55. `TASK-155` 検査/設定修正 + 拡張項目自由化（複数指定）
56. `TASK-156` 設定表示APIのPrisma delegate未生成時500エラー修正
57. `TASK-157` 緊急公開画面にワンタップ導線追加
58. `TASK-158` 緊急公開画面の視認性改善（救急モードUI）
59. `TASK-159` ペット詳細ページの情報密度最適化（セクションナビ）
60. `TASK-160` 共通フィードバックUI（成功/失敗トースト）導入
61. `TASK-161` ローディング/スケルトンの統一
62. `TASK-162` Empty State改善（次アクション導線つき）
63. `TASK-163` 通知設定UI（投薬リマインダー実利用化）
64. `TASK-164` 変更履歴（監査ログ）表示
65. `TASK-165` データ共有出力（PDF/印刷向け）
66. `TASK-166` 初回オンボーディング導線
67. `TASK-167` PR作成〜mainマージ運用の段階ガード化（実態厳密）
68. `TASK-168` Post-Mergeチェック項目のCI強制ガード追加
69. `TASK-169` 変更履歴の時刻精度改善（updatedAt基準化）
70. `TASK-170` 投薬リマインダーの定期送信ジョブ化
71. `TASK-171` 緊急公開向け入力品質のバリデーション強化
72. `TASK-172` GitHub Actions Nodeランタイム更新（deprecation対応）
73. `TASK-173` 投薬リマインダー日次判定のタイムゾーン基準化
74. `TASK-174` Seedデータ再設計（2名・10匹のケース拡充）
75. `TASK-175` 課金必須化（30日トライアル/680円）と世帯自動作成・seed削除導線整備
76. `TASK-176` 商用化運用ドキュメント整備（法務/運用/事業/チェックリスト）
77. `TASK-177` サブスク課金 UI/UX 刷新（設定画面・転換率最適化）
78. `TASK-178` 再ログイン後の設定画面取得失敗修正（display設定404回避・表示名方針固定）
79. `TASK-179` OWNER不在防止（OWNER→FAMILY降格ガード）
80. `TASK-180` OWNER不在復旧（最古メンバー限定）+ 監査ログDB保存
81. `TASK-181` 緊急情報保存エラー修正（Promise params / バリデーション表示 / 電話正規化）
82. `TASK-182` ペット情報拡張とQR/写真不具合修正（去勢・飼い主情報・QR URL）
83. `TASK-183` 写真アップロード連鎖不具合修正（Promise params / bucket自動作成 / エラー可視化）
84. `TASK-184` ペット削除（ハード削除）
85. `TASK-185` 医療書類写真登録 + OCR抽出MVP
86. `TASK-186` Prisma build failure修正（shadow DB設定除外）
87. `TASK-187` APIエラーハンドリング統一化
88. `TASK-188` アクセシビリティ改善（WCAG準拠）
89. `TASK-189` モバイルタッチターゲット拡大
90. `TASK-190` フォームバリデーション改善
91. `TASK-191` カスタム確認ダイアログ
92. `TASK-192` ペット詳細ページ情報整理
93. `TASK-193` プログレス表示視覚化
94. `TASK-194` ダークモード対応
95. `TASK-195` コンテキストヘルプ追加
96. `TASK-196` ペット詳細取得失敗と設定画面認証遅延の修正
97. `TASK-197` ペット詳細ページのサーバーエラー修正
98. `TASK-198` 拡張緊急フィールドのデータ登録＆緊急公開画面UIの拡張
99. `TASK-199` ログインAPIのレート制限とFail-Open実装
100. `TASK-200` アカウント削除（退会）機能の実装
101. `TASK-201` パスワードリセット機能の実装
102. `TASK-202` 緊急QRボタンをペット詳細画面の最上部に移動
103. `TASK-203` ペット一覧カードに「緊急」ボタン追加
104. `TASK-204` 緊急公開画面の情報優先度改善
105. `TASK-205` 管理画面MVP（ユーザー一覧・課金状態確認）
106. `TASK-206` エラーモニタリング（Sentry等）導入
67. `TASK-137` ペット一覧/詳細の実データ化（モック撤去）
68. `TASK-136` ペット新規登録導線
69. `TASK-135` ペット基本情報編集導線
70. `TASK-134` 緊急情報・投薬の編集導線実装
71. `TASK-133` 家族招待コード発行・共有導線
72. `TASK-132` ログイン失敗時のランタイム例外表示修正
73. `TASK-131` 新規ユーザー登録導線の追加
74. `TASK-130` 公開緊急RPC未配置時のフォールバック対応
75. `TASK-129` TASK完了時のIssueクローズ必須化
76. `TASK-128` TASK作成時のIssue必須化と並行実装向け分割方針の追加
77. `TASK-127` 公開緊急導線の整合化（QRトークン/ドキュメント/E2E）
78. `TASK-126` 実装運用の必須ガード（AGENTS/PRテンプレ/CI）
79. `TASK-125` health APIのpetId UUIDバリデーション強化（500→400）
80. `TASK-124` Prisma adapter未設定による500エラー修正
81. `TASK-123` 健康トラッキングのテスト拡充（unit/integration/e2e）
82. `TASK-122` 健康トラッキングのグラフ可視化UI
83. `TASK-121` 健康トラッキングの入力UI
84. `TASK-120` 健康トラッキングAPI/スキーマ整備
85. `TASK-119` 実DB統合テスト + CI記載更新
86. `TASK-118` ワクチン履歴編集フロー
87. `TASK-117` 投薬リマインダー実送信
88. `TASK-116` 医療記録追加フォーム永続化
89. `TASK-115` 写真Storageアップロード接続
90. `TASK-114` QRトークン再生成UI連携
91. `TASK-113` 公開緊急APIのRPC移行
92. `TASK-112` 認証/招待ユーザー連携
93. `TASK-111` 疾患非依存の健康トラッキング + グラフ表示
94. `TASK-110` 基盤セットアップ（Next.js + Supabase + Prisma）
95. `TASK-109` 認証 + 家族招待コード基盤
96. `TASK-108` スキーマ + RLS実装
97. `TASK-107` ペットプロフィール + 写真管理
98. `TASK-106` 緊急情報 + QR公開画面
99. `TASK-105` 医療タイムライン
100. `TASK-104` 投薬管理 + カレンダー
101. `TASK-103` ワクチン・予防歴
102. `TASK-102` 統合テスト（unit/integration/e2e）
103. `TASK-101` ドキュメント整備
